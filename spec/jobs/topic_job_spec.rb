...
RSpec.describe TopicJob, type: :job do
  it 'Updates Categories, Topics, Levels, Tasks, and Goals' do
    VCR.use_cassette('topic_job') do
      ...
      expect(TopicLevelTask.all.size).to eq(0)
      expect(TopicLevelGoal.all.size).to eq(0)
      ...
      expect(TopicLevelTask.all.size).to be > 0
      expect(TopicLevelGoal.all.size).to be > 0
    end
  end
end
